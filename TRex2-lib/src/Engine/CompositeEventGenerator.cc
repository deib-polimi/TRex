...
PubPkt * CompositeEventGenerator::generateCompositeEvent(PartialEvent *partialEvent, map<int, Aggregate *> &aggregates, int aggsSize[MAX_RULE_FIELDS], map<int, vector<PubPkt *> > &receivedPkts, map<int, vector<PubPkt *> > &receivedAggs, map<int, set<CPUParameter *> > &aggregateParameters) {
...
inline int CompositeEventGenerator::computeIntValue(PartialEvent *partialEvent, map<int, Aggregate *> &aggregates, int aggsSize[MAX_RULE_FIELDS], map<int, vector<PubPkt *> > &receivedPkts, map<int, vector<PubPkt *> > &receivedAggs, map<int, set<CPUParameter *> > &aggregateParameters, OpTree *opTree) {
	...
	if (type==LEAF) {
		...
		cout << "CPU not leaf " << leftValue << "; " << rightValue << ". OP " << op << endl;
...
inline float CompositeEventGenerator::computeFloatValue(PartialEvent *partialEvent, map<int, Aggregate *> &aggregates, int aggsSize[MAX_RULE_FIELDS], map<int, vector<PubPkt *> > &receivedPkts, map<int, vector<PubPkt *> > &receivedAggs, map<int, set<CPUParameter *> > &aggregateParameters, OpTree *opTree) {
	OpTreeType type = opTree->getType();
	if (type==LEAF) {
		OpValueReference *reference = opTree->getValueReference();
		RulePktValueReference *pktReference = dynamic_cast<RulePktValueReference *>(reference);
...
inline bool CompositeEventGenerator::computeBoolValue(PartialEvent *partialEvent, map<int, Aggregate *> &aggregates, int aggsSize[MAX_RULE_FIELDS], map<int, vector<PubPkt *> > &receivedPkts, map<int, vector<PubPkt *> > &receivedAggs, map<int, set<CPUParameter *> > &aggregateParameters, OpTree *opTree) {
	OpTreeType type = opTree->getType();
	if (type==LEAF) {
		OpValueReference *reference = opTree->getValueReference();
		RulePktValueReference *pktReference = dynamic_cast<RulePktValueReference *>(reference);
...
inline void CompositeEventGenerator::computeStringValue(PartialEvent *partialEvent, map<int, Aggregate *> &aggregates, int aggsSize[MAX_RULE_FIELDS], map<int, vector<PubPkt *> > &receivedPkts, map<int, vector<PubPkt *> > &receivedAggs, map<int, set<CPUParameter *> > &aggregateParameters, OpTree *opTree, char *result) {
...
inline float CompositeEventGenerator::computeAggregate(int index, PartialEvent *partialEvent, map<int, Aggregate *> &aggregates, int aggsSize[MAX_RULE_FIELDS], map<int, vector<PubPkt *> > &receivedPkts, map<int, vector<PubPkt *> > &receivedAggs, map<int, set<CPUParameter *> > &aggregateParameters) {
	...
	map<int, set<CPUParameter *> >::iterator paramIt = aggregateParameters.find(index);
...
// 		  if (! checkComplexParameter(pkt, partialEvent, paramIt->second, -1, false)) continue;
		}
		float val = 0;
...
bool CompositeEventGenerator::checkParameters(PubPkt *pkt, PartialEvent *partialEvent, set<CPUParameter *> &parameters) {
	for (set<CPUParameter *>::iterator it=parameters.begin(); it!=parameters.end(); ++it) {
// 	  cout << "Agg par" << endl;
	  if (! checkComplexParameter(pkt, partialEvent, *it, -1, AGG)) return false;
	}
	return true;
}
