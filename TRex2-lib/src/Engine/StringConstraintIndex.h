...
typedef struct StringOperatorsTable {
  ...
  std::map<std::string, StringTableConstraint*> eq;
  // Value -> include contraint
  std::map<std::string, StringTableConstraint*> in;
  // Overriding
...
private:
  ...
  std::map<std::string, StringOps> indexes;
  // Set of all constraints used in the table
  std::set<StringTableConstraint*> usedConstraints;
   ...
   * the mh structures
   */
  inline void processConstraint(StringTableConstraint* c, MatchingHandler& mh,
                                std::map<TablePred*, int>& predCount);
};
