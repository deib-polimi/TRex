...
// 	std::map<int, std::set<Parameter *> > negationParameters;			// Parameters in the rule to check in the meantime (negation id -> data structure)
	std::map<int, std::set<CPUParameter *> > negationComplexParameters;			// Parameters in the rule to check in the meantime (negation id -> data structure)
 	std::map<int, std::set<CPUParameter *> > aggregateComplexParameters;		// Parameters in the rule to check in the meantime (aggregate id -> data structure)
	...
	inline void addComplexParameter(Op pOperation, OpTree *lTree, OpTree *rTree, int lastIdx, StateType type, ValType vtype);
	
	int computeIntValue(PubPkt *pkt, PartialEvent *partialEvent, OpTree *opTree, int index, bool isNeg);
...
// 	inline bool checkComplexParameter(PubPkt *pkt, PartialEvent *partialEvent, CPUParameter *parameter, int index, bool isNeg);

	/**
	 * Returns true if all parameters are satisfied by the packet
	 */
	inline bool checkParameters(PubPkt *pkt, PartialEvent *partialEvent, std::set<CPUParameter *> &complexParameters, int index, StateType sType);
