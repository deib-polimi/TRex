...
IntConstraintIndex::~IntConstraintIndex() {
	for (set<IntTableConstraint *>::iterator it=usedConstraints.begin(); it!=usedConstraints.end(); it++) {
		IntTableConstraint *itc = *it;
		delete itc;
	}
}

void IntConstraintIndex::installConstraint(Constraint &constraints, TablePred *predicate) {
...
void IntConstraintIndex::processMessage(PubPkt *pkt, MatchingHandler &mh, map<TablePred *, int> &predCount) {
	for (int i=0; i<pkt->getAttributesNum(); i++) {
		if (pkt->getAttribute(i).type!=INT) continue;
