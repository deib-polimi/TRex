...
StacksRule::StacksRule(RulePkt *pkt) {
	...
	if (pkt->getCompositeEventTemplate()->getAttributesNum() + pkt->getCompositeEventTemplate()->getStaticAttributesNum() ==0) {
	...
	// Initialize parameters belonging to the rule
 	for (int i=0; i<pkt->getParametersNum(); i++) {
 	 	addParameter(pkt->getParameter(i).evIndex2, pkt->getParameter(i).name2, pkt->getParameter(i).evIndex1, pkt->getParameter(i).name1, pkt->getParameter(i).type,pkt);
 	}
	
	for (int i=0; i<pkt->getComplexParametersNum(); i++) {
		 addComplexParameter(pkt->getComplexParameter(i).operation, pkt->getComplexParameter(i).leftTree, pkt->getComplexParameter(i).rightTree, pkt->getComplexParameter(i).lastIndex, pkt->getComplexParameter(i).type, pkt->getComplexParameter(i).vtype);
	}
...
StacksRule::~StacksRule() {
	...
	for (map<int, set<CPUParameter *> >::iterator it=branchStackComplexParameters.begin(); it!=branchStackComplexParameters.end(); ++it) {
		set<CPUParameter *> temp =  it->second;
		for(set<CPUParameter*>::iterator it2=temp.begin(); it2!=temp.end();++it2){
			CPUParameter *par= *it2;
			delete par;
		}
	}

	for (map<int, set< CPUParameter *> >::iterator it=aggregateComplexParameters.begin(); it!=aggregateComplexParameters.end(); ++it) {
		set<CPUParameter *> temp =  it->second;
		for(set<CPUParameter*>::iterator it2=temp.begin(); it2!=temp.end();++it2){
			CPUParameter *par= *it2;
			delete par;
...
void StacksRule::addComplexParameter(Op pOperation, OpTree *lTree, OpTree *rTree, int lastIdx, StateType type, ValType vtype) {
...
void StacksRule::getWinEvents(list<PartialEvent *> *results, int index, TimeMs tsUp, CompKind mode, PartialEvent *partialEvent) {
	...
	map<int, set<CPUParameter *> >::iterator itComplex = branchStackComplexParameters.find(index);
	if (itComplex!=branchStackComplexParameters.end()) useComplexParameters = true;
	...
	while (true) {
		...
		if (useComplexParameters) usable = checkParameters(tmpPkt, partialEvent, itComplex->second, index, STATE);
		if (usable) {
...
bool StacksRule::checkNegation(int negIndex, PartialEvent *partialResult) {
	...
	for (int count=0; count<=index2-index1; count++) {
		...
		if (checkParameters(tmpPkt, partialResult, itComplex->second, negIndex, NEG)) return true;
	}
	return false;
}
...
bool StacksRule::checkParameters(PubPkt *pkt, PartialEvent *partialEvent, set<CPUParameter *> &complexParameters, int index, StateType sType) {
	for (set<CPUParameter *>::iterator it=complexParameters.begin(); it!=complexParameters.end(); ++it) {
		CPUParameter *par = *it;
   		if (! checkComplexParameter(pkt, partialEvent, par, index, sType)) return false;
	}
	return true;
}
...
void StacksRule::createComplexEvents(list<PartialEvent *> *partialEvents, set<PubPkt *> &results) {
	for (list<PartialEvent *>::iterator it=partialEvents->begin(); it!=partialEvents->end(); ++it) {
		...
		if (compositeEventId>=0) {
			...
			genPkt = eventGenerator->generateCompositeEvent(pe, aggregates, aggsSize, receivedPkts, receivedAggs, aggregateComplexParameters);
		}
