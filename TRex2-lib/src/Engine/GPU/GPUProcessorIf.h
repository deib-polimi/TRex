...
/**
 * This represents the part of a GPUProcessor that is exposed to the Cuda Kernels.
 */
...
public:
	 ...
	 */
	virtual int getMinValidElement(int stack, uint64_t minTimestamp, StateType t) = 0;

	/**
	 * Returns the maximum element in the given stack having a time stamp greater than maxTimestamp.
	 * If agg==true, stack represents an aggregate stack.
	 */
	virtual int getMaxValidElement(int stack, uint64_t maxTimestamp, StateType t) = 0;
	...
	virtual bool getAttributeIndexAndType(int state, char *name, int &index, ValType &type, StateType stype) = 0;
