...
/***
 * This is the memory manager for the CUDA engine. Only one instance is created that is shared among all the GPUProcessors created by the GPUEngine.
 */
class MemoryManager
{
...
public:
    ...
    /**
     * Must be called by a CudaKernel in order to start using the GPU memory and the memory manager services;
     * returns a unique int representing the subscriber
     */
    int subscribe(int seqLen, int aggrNum, int negNum, int paramNum);

    /**
     * Must be called by a CudaKernel when it stops using the GPU; releases the resources allocated
     */
    ...
    /**
     * Requires the identification token of the calling CudaKernel, the state (stack number)
     * Returns the indexes of the pages representing the specified state of the specified CudaKernel
     */
    void getPageIndexes(int id, int state, passer &res);

    /**
     * Requires the identification token of the calling CudaKernel
     * Returns the pointer to the EventInfoSet representing the previousResults array on the device
     */
    EventInfoSet *getPrevResultsPtr(int id);

    /**
     * Requires the identification token of the calling CudaKernel
     * Returns the pointer to the EventInfoSet representing the currentResults array on the device
     */
    ...
    /**
     * Requires the identification token of the calling CudaKernel
     * Restores the previousResults and currentResults pointers to the original state
     */
    ...
    /**
     * Requires the identification token of the calling CudaKernel, the state (stack number)
     * Returns the indexes of the pages representing the specified negation of the specified CudaKernel
     */
    ...
    /**
     * Requires the identification token of the calling CudaKernel, the state (stack number)
     * Returns the indexes of the pages representing the specified aggregate of the specified CudaKernel
     */
    ...
    /**
     * Adds ev to the stack stack of the type kind, in position index, for the CudaKernel id
     * Copies asyncronously on the fly if the corresponding page on the GPU is already used by id, else ev is only added in the HOST memory, waiting for the terminator
     */
    void addEventToStack(int id, EventInfo ev, int stack, int index, StateType type);

    /**
     * Returns the event from the rule id, stack stack and index index; this is only for STATE events, so no AGG nor NEG.
     */
    EventInfo getEvent(int id, int stack, int index);

    /**
     * Removes the STATE event from the rule id, stack stack and index index
     * It operates on the HOST memory only, but it invalidates the corresponding page on the GPU
     */
    ...
    /*
     * Returns a pointer to the float array used to compute aggregates used by rule id
     */
...
private:
    ...
    /*
     * Returns the index of the first chunk of memory available for a new assignment
     */
    ...
    /**
     * Copies page page to the device memory and updates informations about memory pages on GPU
     */
