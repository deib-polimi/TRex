...
void GPUProcessor::getConstraint(int state, int num, int  &idx, Op &o, int &val) {
    Constraint c = rulePkt->getPredicate(sequenceLen -1 - state).constraints[num];
    o = c.op;
    val = c.intVal;
    idx = 0;
}
...
int GPUProcessor::getCompositeEventId()
{
    return ceTemplate->getEventType();
}

CompositeEventTemplate* GPUProcessor::getCompositeEvent()
{
    return ceTemplate;
}

int GPUProcessor::getAttributesNum()
{
    return ceTemplate->getAttributesNum();
}
...
int GPUProcessor::getSequenceLen()
{
    return sequenceLen;
}

int GPUProcessor::getNumParam()
{
    return rulePkt->getParametersNum();
}
...
int GPUProcessor::getAggregatesNum()
{
    return rulePkt->getAggregatesNum();
}
...
int GPUProcessor::getNegationsNum()
{
    return rulePkt->getNegationsNum();
}

Negation *GPUProcessor::getNegation(int num)
{
    return negations[num];
}

int GPUProcessor::getNegsSize(int negIndex)
{
    return negsSize[negIndex];
}


void GPUProcessor::addNegation(int eventType, Constraint constraints[], int constrLen, int lowIndex, TimeMs &lowTime, int highIndex) {
...
void GPUProcessor::init() {
    ...
    mmToken = mm->subscribe(sequenceLen, rulePkt->getAggregatesNum(), rulePkt->getNegationsNum(), rulePkt->getComplexParametersNum());
    ...
    for (int i=0; i<rulePkt->getNegationsNum(); i++) {
        ...
        highIndexesNegs[i] = 0;
        negsSize[i]=0;
        addNegation(rulePkt->getNegation(i).eventType, rulePkt->getNegation(i).constraints, rulePkt->getNegation(i).constraintsNum, rulePkt->getNegation(i).lowerId, rulePkt->getNegation(i).lowerTime, rulePkt->getNegation(i).upperId);

        map<int, set<int> >::iterator it = negationsTopics.find(rulePkt->getNegation(i).eventType);
        if (it==negationsTopics.end()) {
            set<int> emptySet;
            emptySet.insert(i);
            negationsTopics.insert(make_pair(rulePkt->getNegation(i).eventType, emptySet));
        } else {
            it->second.insert(i);
        }
...
bool GPUProcessor::checkDefinitions()
{
    ...
    if (rulePkt->getAggregatesNum() > MAX_NUM_AGGREGATES) {
        cout << "You need to raise MAX_NUM_AGGREGATES to " << rulePkt->getAggregatesNum() << " to perform this computation. Exiting" << endl;
        return false;
    }
    if (rulePkt->getNegationsNum() > MAX_NEGATIONS_NUM) {
        cout << "You need to raise MAX_NEGATIONS_NUM to " << rulePkt->getNegationsNum() << " to perform this computation. Exiting" << endl;
        return false;
    }
    if (rulePkt->getPredicatesNum() > MAX_RULE_FIELDS) {
        cout << "You need to raise MAX_RULE_FIELDS to " << rulePkt->getPredicatesNum() << " to perform this computation. Exiting" << endl;
        return false;
    }
    if (negationsParametersNum > MAX_PARAMETERS_NUM || aggregateParametersNum > MAX_PARAMETERS_NUM || stateParamtersNum > MAX_PARAMETERS_NUM) {
        cout << "You need to raise MAX_PARAMETERS_NUM to perform this computation. Exiting" << endl;
        return false;
    }
    return true;
}
...
void GPUProcessor::processEvent(PubPkt *event, set<PubPkt *> &generatedEvents) {
    ...
    for (list<int>::iterator it=states.begin(); it!=states.end(); ++it) {
        int state = *it;
        if (!checkConstraints(event, rulePkt, sequenceLen - state -1)) return;
    }

    for (list<int>::iterator it=negs.begin(); it!=negs.end(); ++it) {
        int neg = *it;
        if (states.size()==0 && !checkNegationConstraints(event, rulePkt, neg)) return;
        addEventToNegationStack(neg, event);
    }

    for (list<int>::iterator it=aggs.begin(); it!=aggs.end(); ++it) {
        int agg = *it;
        if (states.size()==0 && !checkAggregateConstraints(event, rulePkt, agg)) return;
    ...
    for (list<int>::iterator it=states.begin(); it!=states.end(); ++it) {
        ...
        // The event matches the last state
            ...
            if (! deleteExpiredEvents(event)) break;
		...
		GPUProcessor *thisProc = this;
                ...
                if (kernels->setParameters(parameters) && kernels->setAggregatesParameters(aggregatesParameters) && kernels->setNegationsParameters(negationsParameters)) first = false;
            }
...
{
    if (t==STATE) {
        return timestamps[stack][highIndexes[stack]-1];// getFirstValidElementCircular(timestamps[stack], lowIndexes[stack], highIndexes[stack], minTimestamp);
    } else if (t==AGG) {
        return aggTimestamps[stack][highIndexesAggs[stack]-1];//return getFirstValidElementCircular(aggTimestamps[stack], lowIndexesAggs[stack], highIndexesAggs[stack], minTimestamp);
    }
    else if (t==NEG) {
...
int GPUProcessor::getMinValidElement(int stack, uint64_t minTimestamp, StateType t) {
    if (t==STATE) {
        return getFirstValidElementCircular(timestamps[stack], lowIndexes[stack], highIndexes[stack], minTimestamp);
    } else if (t==AGG) {
        return getFirstValidElementCircular(aggTimestamps[stack], lowIndexesAggs[stack], highIndexesAggs[stack], minTimestamp);
    } else if (t==NEG) {
        return getFirstValidElementCircular(negTimestamps[stack], lowIndexesNegs[stack], highIndexesNegs[stack], minTimestamp);
    }
    //Suppresses gcc warning, should never be reached
    return -1;
}
...
int GPUProcessor::getMaxValidElement(int stack, uint64_t maxTimestamp, StateType t) {
    if (t==STATE) {
        return getLastValidElementCircular(timestamps[stack], lowIndexes[stack], highIndexes[stack], maxTimestamp, lowIndexes[stack]);
    } else if (t==AGG) {
        return getLastValidElementCircular(aggTimestamps[stack], lowIndexesAggs[stack], highIndexesAggs[stack], maxTimestamp, lowIndexesAggs[stack]);
    }
    else if (t==NEG) {
        return getLastValidElementCircular(negTimestamps[stack], lowIndexesNegs[stack], highIndexesNegs[stack], maxTimestamp, lowIndexesNegs[stack]);
    }
...
int GPUProcessor::getNumParamAggregates()
{
    return aggregateParametersNum;
}

int GPUProcessor::getNumParamNegations()
{
    return negationsParametersNum;
}
...
void GPUProcessor::addParameter(GPUParameter *parameterp) {
	...
	int state = parameter->lastIndex;
        ...
        map<int, set<GPUParameter *> >::iterator it = aggregatesParameters.find(state);
...
{
    return sequenceLen - 1 - rulePkt->getPredicate(sequenceLen - 1 -state).refersTo;
}


bool GPUProcessor::getAttributeIndexAndType(int state, char* name, int& index, ValType& type, StateType stype)
{
    switch (stype) {
    case STATE:
        if (samples.find(sequenceLen - state - 1) == samples.end()) return false;
        return samples[sequenceLen - state - 1]->getAttributeIndexAndType(name, index, type);
        break;

    case AGG:
        if (aggSamples.find(state) == aggSamples.end()) return false;
        return aggSamples[state]->getAttributeIndexAndType(name, index, type);
        break;

    case NEG:
        if (negSamples.find(state) == negSamples.end()) return false;
...
bool GPUProcessor::compareEvt(EventInfo e1, EventInfo e2, int size)
{
    ...
    for (int i=0; i<size; i++) {
        if (e1.attr[i].type != e2.attr[i].type) return false;
        switch (e1.attr[i].type) {
        case INT:
            if (e1.attr[i].intVal != e2.attr[i].intVal) return false;
            break;

        case FLOAT:
            if (e1.attr[i].floatVal != e2.attr[i].floatVal) return false;
            break;

        case BOOL:
            if (e1.attr[i].boolVal != e2.attr[i].boolVal) return false;
            break;

        case STRING:
            if (strcmp(e1.attr[i].stringVal, e2.attr[i].stringVal)!=0) return false;
            break;
        }
    }
    return true;
...
{
    for (int i=lowIndexes[stack]; i<highIndexes[stack]; i=(i+1)%ALLOC_SIZE) {
        if (compareEvt(mm->getEvent(mmToken, stack, i), ev, samples[stack]->getAttributesNum())) {
...
inline bool GPUProcessor::deleteExpiredEvents(PubPkt *event) {
    ...
    for (int stack=sequenceLen-2; stack>=0; stack--) {
        int refersToStack = sequenceLen - 1 - rulePkt->getPredicate(sequenceLen - stack -1).refersTo;

        if (highIndexes[stack] - lowIndexes[stack]==0) continue;
        if (refersToStack == sequenceLen - 1) {
            //It's the terminator!
            minTimeStamp = event->getTimeStamp().getTimeVal() - rulePkt->getPredicate(sequenceLen - 1 - stack).win.getTimeVal();
        }
        else {
            if (timestamps[refersToStack][lowIndexes[refersToStack]] > rulePkt->getPredicate(sequenceLen - 1 - stack).win.getTimeVal()) {
                minTimeStamp = timestamps[refersToStack][lowIndexes[refersToStack]] - rulePkt->getPredicate(sequenceLen - 1 - stack).win.getTimeVal();
            } else minTimeStamp = 0;
        }
        deleteInvalidElementsCircular(timestamps[stack], lowIndexes[stack], highIndexes[stack], minTimeStamp);
    }
    ...
    for (int negIndex=0; negIndex<negsNum; negIndex++) {
        ...
        if (refersToStack == sequenceLen - 1) {
            ...
            if (timestamps[refersToStack][lowIndexes[refersToStack]] > win) {
                minTimeStamp = timestamps[refersToStack][lowIndexes[refersToStack]] - win;
            } else minTimeStamp = 0;
        }
        deleteInvalidElementsCircular(negTimestamps[negIndex], lowIndexesNegs[negIndex], highIndexesNegs[negIndex], minTimeStamp);
    }
    ...
    for (int aggIndex=0; aggIndex<aggsNum; aggIndex++) {
        ...
        deleteInvalidElementsCircular(aggTimestamps[aggIndex], lowIndexesAggs[aggIndex], highIndexesAggs[aggIndex], minTimeStamp);
    }
