...
private:
  ...
  std::map<int, IntConstraintIndex> intIndex;
  // EventType -> Index for constraints on float values
  std::map<int, FloatConstraintIndex> floatIndex;
  // EventType -> Index for constraints on boolean values
  std::map<int, BoolConstraintIndex> boolIndex;
  // EventType -> Index for constraints on string values
  std::map<int, StringConstraintIndex> stringIndex;
  // EventType -> Index of "no" constraints
  std::map<int, NoConstraintIndex> noIndex;
   ...
   */
  inline void installTablePredicate(int eventType, Constraint* constraint,
                                    int numConstraints, TablePred* tp);
  inline void installNoConstraint(int eventType, TablePred* tp);
  inline void installConstraint(int eventType, Constraint& constr,
                                TablePred* tp);
};
