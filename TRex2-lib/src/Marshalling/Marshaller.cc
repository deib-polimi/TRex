...
int Marshaller::getSize(PubPkt* pkt) { return getNumBytes(pkt) + 5; }

int Marshaller::getSize(RulePkt* pkt) { return getNumBytes(pkt) + 5; }

int Marshaller::getSize(SubPkt* pkt) { return getNumBytes(pkt) + 5; }

int Marshaller::getSize(AdvPkt* pkt) { return getNumBytes(pkt) + 5; }

int Marshaller::getSize(JoinPkt* pkt) { return getNumBytes(pkt) + 5; }
...
int Marshaller::encode(RulePkt* source, char* dest, int startIndex) {
  ...
  CompositeEventTemplate* compositeEventTemplate =
      source->getCompositeEventTemplate();
...
int Marshaller::getNumBytes(Constraint& constraint) {
  ...
  if (constraint.type == INT)
    size += getNumBytes(constraint.intVal);
  else if (constraint.type == FLOAT)
    size += getNumBytes(constraint.floatVal);
  else if (constraint.type == BOOL)
    size += getNumBytes(constraint.boolVal);
  else if (constraint.type == STRING)
    size += getNumBytes(constraint.stringVal);
  return size;
}
...
int Marshaller::encode(Constraint& source, char* dest, int startIndex) {
  ...
  if (source.type == INT)
    startIndex = encode(source.intVal, dest, startIndex);
  if (source.type == FLOAT)
    startIndex = encode(source.floatVal, dest, startIndex);
  if (source.type == BOOL)
    startIndex = encode(source.boolVal, dest, startIndex);
  if (source.type == STRING)
    startIndex = encode(source.stringVal, dest, startIndex);
  return startIndex;
}
...
int Marshaller::getNumBytes(Attribute& source) {
  ...
  if (source.type == INT)
    size += getNumBytes(source.intVal);
  else if (source.type == FLOAT)
    size += getNumBytes(source.floatVal);
  else if (source.type == BOOL)
    size += getNumBytes(source.boolVal);
  else if (source.type == STRING)
    size += getNumBytes(source.stringVal);
  return size;
}
...
int Marshaller::encode(CompositeEventTemplate& source, char* dest,
                       int startIndex) {
  startIndex = encode(source.getEventType(), dest, startIndex);
  int numAttributes = source.getAttributesNum();
...
int Marshaller::encode(CompositeEventTemplateAttr& source, char* dest,
                       int startIndex) {
...
int Marshaller::encode(OpTree* source, char* dest, int startIndex) {
  ...
  if (source->getType() == INNER) {
    ...
    startIndex = encode((RulePktValueReference*)source->getValueReference(),
                        dest, startIndex);
  }
  return startIndex;
}
...
int Marshaller::encode(RulePktValueReference* source, char* dest,
                       int startIndex) {
...
int Marshaller::getNumBytes(bool source) { return 1; }
...
int Marshaller::getNumBytes(int source) { return 4; }
...
int Marshaller::getNumBytes(uint64_t source) { return 8; }
...
int Marshaller::getNumBytes(long source) { return 8; }
...
int Marshaller::getNumBytes(float source) { return 4; }
...
int Marshaller::getNumBytes(char* source) { return strlen(source) + 4; }
...
int Marshaller::getNumBytes(CompKind source) { return 1; }

int Marshaller::encode(CompKind source, char* dest, int startIndex) {
  if (source == EACH_WITHIN)
    dest[startIndex++] = 0;
  else if (source == FIRST_WITHIN)
    dest[startIndex++] = 1;
  else if (source == LAST_WITHIN)
    dest[startIndex++] = 2;
  else if (source == ALL_WITHIN)
    dest[startIndex++] = 3;
  return startIndex;
}

int Marshaller::getNumBytes(Op source) { return 1; }
...
int Marshaller::getNumBytes(StateType source) { return 1; }
...
int Marshaller::getNumBytes(ValType source) { return 1; }
...
int Marshaller::getNumBytes(AggregateFun source) { return 1; }
...
int Marshaller::getNumBytes(OpTreeType source) { return 1; }
...
int Marshaller::getNumBytes(OpTreeOperation source) { return 1; }
...
int Marshaller::getNumBytes(PktType source) { return 1; }

int Marshaller::encode(PktType source, char* dest, int startIndex) {
  if (source == PUB_PKT)
    dest[startIndex] = 0;
  else if (source == RULE_PKT)
    dest[startIndex] = 1;
  else if (source == SUB_PKT)
    dest[startIndex] = 2;
  else if (source == ADV_PKT)
    dest[startIndex] = 3;
  else if (source == JOIN_PKT)
    dest[startIndex] = 4;
  return startIndex + 1;
}

int Marshaller::getNumBytes(TimeMs source) { return 8; }
