...
public:
   ...
   */
  PubPkt* getPkt() { return pkt; }

  /**
   * Returns the set of clients the packet has to be sent to
   */
  std::set<int>* getClients() { return clients; }
...
public:
   ...
   */
  TRexServer(TRexEngine* engine, int port, GenericRoutingTable* routingTable,
             int queueSize);
  ...
  /**
   * Reads from the input queue and sends each received message to the T-Rex
   * engine.
   */
...
private:
  ...
  TRexEngine* engine;
  // The routing table used to store subscriptions
  GenericRoutingTable* routingTable;
  // The maximum size of the queue for input events
  int queueSize;
  // Input queue for received events
  std::queue<PubPkt*> inputQueue;
  // Mutex to access the input and output socket maps
  pthread_mutex_t* socketsMutex;
  // Client id -> input socket for the client
  std::map<int, int> inputSockets;
  // Client id -> output socket for the client
  std::map<int, int> outputSockets;
  // Output queue with the messages for clients
  std::queue<Outbox*> outputQueue;
  // Socket used to listen for new connections
  int serverSocket;
  // Boolean variable used to stop the server (through the stop() method)
  bool running;
  // The port the server listen to
  int port;
  // Types of events that need to be re-submitted to the engine
  // for further processing
  std::set<int> recursiveTypes;

  // Condition variable for the input queue
  pthread_cond_t* inputCond;
  // Mutex for the input queue
  pthread_mutex_t* inputMutex;
  // Mutex for the output queues
  pthread_mutex_t* outputMutex;
  // Condition variable for the output queues
  pthread_cond_t* outputCond;
  // Client id -> thread reading information coming from the client
  std::map<int, pthread_t*> readingThreads;
  // Thread writing information to the client
  pthread_t* writingThread;
  // Thread reading information from the input queue
  // and passing it to the T-Rex engine
  ...
  /**
   * Removes the client and all associated data (i.e. mutexes, condition
   * variables, and sockets)
   * reading==true if the function is called within the readingThread,
...
/**
 * Start the thread to read from the queue of input events
 * and sends them to the T-Rex engine.
 */
