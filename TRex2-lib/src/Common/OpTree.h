...
/**
 * Represents the type of a tree: it can be a leaf (a value), or an inner node
 * (a subtree)
 */
enum OpTreeType { LEAF = 0, INNER = 1 };

/**
 * Possible operations stored at inner nodes
 */
enum OpTreeOperation { ADD = 0, SUB = 1, MUL = 2, DIV = 3, AND = 4, OR = 5 };
 ...
 * Actual values are stored at leaves: to make the data structure generic
 * enough,
...
public:
   ...
   */
  OpTree(OpTree* leftTree, OpTree* rightTree, OpTreeOperation parOp,
         ValType parValType);
...
private:
  ...
  // Right subtree
  OpTree* rightTree;
  // Reference to the actual value (in case of leaf type)
