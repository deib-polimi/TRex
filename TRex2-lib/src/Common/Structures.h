...
typedef struct ParameterConstraints {
    ...
    char name1[NAME_LEN+1];			// Name of the first attribute involved in the constraint
    int evIndex2;							// Index of the second state involved in the constraint
    char name2[NAME_LEN+1];			// Name of the second attribute involved in the constraint
...
typedef struct ComplexParameterStruct {
    Op operation;
    StateType type;		// Decides whether the parameter should be used during the computation of a normal state, to an aggregate, or to a negation
    ValType vtype;
    OpTree *leftTree;
    OpTree *rightTree;
} ComplexParameter;
...
/**
 * Represents a complex parameter for the CPU engine. It asks operation to be verified between the values (of type vtype) computed analyzing the trees
 */
...
/**
 * The representation of a node of a binary tree for the GPU serialized array; it can be both a static value or a reference to an attribute of an event
 */
...
/**
 * Represents a complex parameter for the GPU engine. It asks operation to be verified between the values (of type vtype) computed analyzing the trees
 */
typedef struct ComplexGPUParameterConstraints {
    Op operation;
    StateType sType;						// Decides whether the second index refers to a normal state, to an aggregate, or to a negation
    ValType vType;
...
typedef struct GNeg {
    ...
    uint64_t lowerTime;					// Time of the lower bound
...
typedef struct Agg {
    ...
    char name[NAME_LEN+1];			// Name of the attribute to use for computation
} Aggregate;
...
typedef struct EventInfoStruct {
    ...
    bool operator==(const EventInfoStruct &x) const {
        ...
        for (int i=0; i<MAX_NUM_ATTR; i++) {
            if (attr[i].intVal != x.attr[i].intVal) return false;
        }
        return true;
    }
...
typedef struct {
    int workspaceNum;
    int *resPtr;									// Results computed during the evaluation of the current column
    int *resultsSizePtr;
    int eventsChunks[MAX_RULE_FIELDS-1][MAX_PAGES_PER_STACK]; // Pointers to the events information in device memory
    int numEventChunks[MAX_RULE_FIELDS];
    int aggregatesChunks[MAX_NUM_AGGREGATES][MAX_PAGES_PER_STACK];
    int numAggregatesChunks[MAX_NUM_AGGREGATES];// Pointers to the events information in device memory stored for aggregates
    ...
    GPUParameter *aggregatesParametersPtr;	// Pointers to the parameters defined for each aggregate in device memory
...
typedef struct {
    //The index of the page to be used. Allows [evt_index,col,rule -> memory page] mapping
...
typedef struct {
    EventInfoSet *prevResultsPtr;														// Results computed during the evaluation of the previous column
