...
bool checkConstraints(PubPkt* event, RulePkt* rule, int state)
{
    ...
    for (int k=0; k < rule->getPredicate(state).constraintsNum; k++) {
        ...
        if (event->getAttributeIndexAndType(rule->getPredicate(state).constraints[k].name, idx, t)==false) return false;
...
bool checkAggregateConstraints(PubPkt* event, RulePkt* rule, int aggNum)
{
    ...
    for (int k=0; k < rule->getAggregate(aggNum).constraintsNum; k++) {
        if (event->getAttributeIndexAndType(rule->getAggregate(aggNum).constraints[k].name, idx, t)==false) return false;
...
bool checkNegationConstraints(PubPkt* event, RulePkt* rule, int negNum)
{
    ...
    for (int k=0; k < rule->getNegation(negNum).constraintsNum; k++) {
        if (event->getAttributeIndexAndType(rule->getNegation(negNum).constraints[k].name, idx, t)==false) return false;
...
void getMatchingColumns(PubPkt *e, map<int, set<int> > &topics, list<int> &results) {
    ...
    for (set<int>::iterator statesIt=it->second.begin(); statesIt!=it->second.end(); ++statesIt) {
        int state = *statesIt;
...
EventInfo createEventInfo(PubPkt *event) {
    ...
    if (event->getAttributesNum() > MAX_NUM_ATTR) {
        cout << "You need to raise MAX_NUM_ATTR to " << event->getAttributesNum() << " to perform this computation. Exiting" << endl;
        exit(-1);
    }
...
int getFirstValidElementCircular(uint64_t *column, int lowIndex, int highIndex, uint64_t minTimeStamp) {
    ...
    if (column[maxValue]<=minTimeStamp) return -1;
    while (relativeMaxValue-relativeMinValue>1) {
        int relativeMidPoint = relativeMinValue+(relativeMaxValue-relativeMinValue)/2;
    ...
    if (relativeMaxValue-relativeMinValue==0) return minValue;
    if (column[minValue]>minTimeStamp) return minValue;
    return maxValue;
}
...
int deleteInvalidElementsCircular(uint64_t *column, int &lowIndex, int highIndex, uint64_t minTimeStamp) {
    ...
    if (firstValidElement==lowIndex) return columnSize;
    lowIndex = firstValidElement;
    int newSize;
    if (highIndex>=lowIndex) newSize = highIndex-lowIndex;
    else newSize = ALLOC_SIZE+highIndex-lowIndex;
    return newSize;
}
...
float computeFloatValueForParameters(PubPkt *pkt, PartialEvent *partialEvent, OpTree *opTree, int index, StateType sType) {
    ...
    if (type==LEAF) {
        ...
        if ((refIndex == index && pktReference->refersToAgg() == 0 && pktReference->refersToNeg() == 0 && sType==STATE) || (pktReference->refersToNeg() && sType==NEG) || (pktReference->refersToAgg() && sType==AGG))
        {
            cPkt = pkt;
        }
        else {
        ...
        int attrIndex;
        ValType vType;
        if (cPkt->getAttributeIndexAndType(pktReference->getName(), attrIndex, vType)==false) return 0;
        if (vType==INT) return cPkt->getIntAttributeVal(attrIndex);
        else if (vType==FLOAT) return cPkt->getFloatAttributeVal(attrIndex);
        else if (vType==BOOL) return cPkt->getBoolAttributeVal(attrIndex);
    } else {
        // Integer can only be obtained from integer: assume this is ensured at rule deployment time
        float leftValue = computeFloatValueForParameters(pkt, partialEvent, opTree->getLeftSubtree(), index, sType);
        float rightValue = computeFloatValueForParameters(pkt, partialEvent, opTree->getRightSubtree(), index, sType);
...
int computeIntValueForParameters(PubPkt *pkt, PartialEvent *partialEvent, OpTree *opTree, int index, StateType sType) {
    ...
    if (type==LEAF) {
        ...
        // Integer can only be obtained from integer: assume this is ensured at rule deployment time
        int leftValue = computeIntValueForParameters(pkt, partialEvent, opTree->getLeftSubtree(), index, sType);
        int rightValue = computeIntValueForParameters(pkt, partialEvent, opTree->getRightSubtree(), index, sType);
        OpTreeOperation op = opTree->getOp();
        if (op==ADD) return leftValue+rightValue;
        if (op==SUB) return leftValue-rightValue;
        if (op==MUL) return leftValue*rightValue;
...
bool computeBoolValueForParameters(PubPkt *pkt, PartialEvent *partialEvent, OpTree *opTree, int index, StateType sType) {
    ...
    if (type==LEAF) {
        ...
        int attrIndex;
        ValType vType;
        if (cPkt->getAttributeIndexAndType(pktReference->getName(), attrIndex, vType)==false) return false;
        if (vType==BOOL) return cPkt->getIntAttributeVal(attrIndex);
        else if (vType==FLOAT) return cPkt->getFloatAttributeVal(attrIndex);
        else if (vType==BOOL) return cPkt->getBoolAttributeVal(attrIndex);
    } else {
        // Integer can only be obtained from integer: assume this is ensured at rule deployment time
        bool leftValue = computeBoolValueForParameters(pkt, partialEvent, opTree->getLeftSubtree(), index, sType);
        bool rightValue = computeBoolValueForParameters(pkt, partialEvent, opTree->getRightSubtree(), index, sType);
...
void computeStringValueForParameters(PubPkt *pkt, PartialEvent *partialEvent, OpTree *opTree, int index, StateType sType, char *res) {
    ...
    if (type==LEAF) {
        ...
        if (pktReference == NULL) {
            StaticValueReference *sReference = dynamic_cast<StaticValueReference *>(reference);
            if (sReference->getType()==STRING) return sReference->getStringValue(res);
        }
        int refIndex = pktReference->getIndex();
        ...
        int attrIndex;
        ValType vType;
        if (cPkt->getAttributeIndexAndType(pktReference->getName(), attrIndex, vType)==false) {
	  strcpy(res, "");
	  return;
	}
        if (vType==STRING) return cPkt->getStringAttributeVal(attrIndex, res);
    } else {
...
bool checkComplexParameter(PubPkt* pkt, PartialEvent* partialEvent, CPUParameter* parameter, int index, StateType sType)
{
    ...
    if (type==INT) {
        int left = computeIntValueForParameters(pkt, partialEvent, parameter->leftTree, index, sType);
        int right = computeIntValueForParameters(pkt, partialEvent, parameter->rightTree, index, sType);
        ...
        float right = computeFloatValueForParameters(pkt, partialEvent, parameter->rightTree, index, sType);
        if (operation==EQ) return left==right;
        if (operation==GT) return left>right;
        if (operation==LT) return left<right;
        if (operation==DF) return left!=right;
